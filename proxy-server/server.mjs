...
import { getAccessToken } from './utils/salesforce-auth.js';
...
app.all('/api/:action', async (req, res) => {
    ...
    try {
        ...
        console.error(`[PROXY ERROR] ${req.method} /api/${action}:`, {
            status,
        ...
        res.status(status).json(typeof errorData === 'string' ? { error: errorData } : errorData);
    }
});
